...
import { Info, ShieldCheck, FileText, Mail, Trash2, ArrowLeft } from 'lucide-react';
...
export default function InfoModal({ isOpen, onClose, onDeleteData }: InfoModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="glass-card sm:max-w-[500px] max-h-[90vh] rounded-[3rem] border-none shadow-2xl p-0 overflow-hidden outline-none flex flex-col">
        <div className="bg-primary/10 p-8 text-center border-b border-white/5 relative shrink-0">
          ...
          <div className="mx-auto w-16 h-16 rounded-2xl bg-primary/20 flex items-center justify-center mb-4">
            <SankalpIcon className="text-primary" size={32} />
          </div>
          <DialogTitle className="text-2xl font-bold font-headline">{t('legal_info')}</DialogTitle>
          <DialogDescription className="mt-2 text-white/40 font-bold uppercase tracking-widest text-[9px]">{t('legal_desc')}</DialogDescription>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
          ...
          <div className="pt-6 border-t border-white/5">
            <Button 
              ...
              className="w-full h-16 rounded-[1.8rem] flex items-center gap-5 px-5 hover:bg-red-500/10 text-red-500 group border border-transparent hover:border-red-500/20"
            >
